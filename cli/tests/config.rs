...
"#).unwrap();
    let cfg = Config::load_from(&path).unwrap();
    let r = resolve(&cfg, &Override { profile: Some("work".into()), ..Default::default() }).unwrap();
...
fn round_trip_preserves_data() {
    ...
    let path = dir.path().join("config.toml");
    let mut cfg = Config::default();
    cfg.profiles.insert("default".into(), Profile { url: "https://x.test".into(), token: "lta_abc".into() });
    cfg.default_profile = Some("default".into());
    cfg.save_to(&path).unwrap();
    let loaded = Config::load_from(&path).unwrap();
    assert_eq!(loaded.default_profile.as_deref(), Some("default"));
    assert_eq!(loaded.profiles.get("default").unwrap().url, "https://x.test");
}
