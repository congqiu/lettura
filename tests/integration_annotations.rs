...
async fn create_and_list_annotations() {
    ...
    let res = app
        .client
        .post(app.url(&format!("/api/v1/entries/{}/annotations", entry_id)))
        .header("Authorization", format!("Bearer {}", token))
    ...
    let res = app
        .client
        .get(app.url(&format!("/api/v1/entries/{}/annotations", entry_id)))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
...
async fn update_annotation() {
    ...
    let res = app
        .client
        .patch(app.url(&format!("/api/v1/annotations/{}", ann_id)))
        .header("Authorization", format!("Bearer {}", token))
...
async fn delete_annotation() {
    ...
    let res = app
        .client
        .delete(app.url(&format!("/api/v1/annotations/{}", ann_id)))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
