...
async fn create_and_list_annotations() {
    ...
    let res = app.client.post(app.url(&format!("/api/v1/entries/{}/annotations", entry_id)))
        .header("Authorization", format!("Bearer {}", token))
    ...
    let res = app.client.get(app.url(&format!("/api/v1/entries/{}/annotations", entry_id)))
...
async fn update_annotation() {
    ...
    let res = app.client.patch(app.url(&format!("/api/v1/annotations/{}", ann_id)))
        .header("Authorization", format!("Bearer {}", token))
...
async fn delete_annotation() {
    ...
    let ann: serde_json::Value = res.json().await.unwrap();
    let ann_id = ann["id"].as_str().unwrap();
    let res = app.client.delete(app.url(&format!("/api/v1/annotations/{}", ann_id)))
        .header("Authorization", format!("Bearer {}", token)).send().await.unwrap();
    assert_eq!(res.status(), 200);
    app.cleanup().await;
}
