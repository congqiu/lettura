...
async fn create_and_list_tagging_rules() {
    ...
    let res = app
        .client
        .get(app.url("/api/v1/tagging-rules"))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
...
async fn update_tagging_rule() {
    ...
    let res = app
        .client
        .post(app.url("/api/v1/tagging-rules"))
        .header("Authorization", format!("Bearer {}", token))
        .json(&json!({
    ...
    let res = app
        .client
        .patch(app.url(&format!("/api/v1/tagging-rules/{}", rule_id)))
        .header("Authorization", format!("Bearer {}", token))
...
async fn delete_tagging_rule() {
    ...
    let res = app
        .client
        .delete(app.url(&format!("/api/v1/tagging-rules/{}", rule_id)))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
...
async fn create_and_list_site_rules() {
    ...
    let res = app
        .client
        .post(app.url("/api/v1/site-rules"))
        .header("Authorization", format!("Bearer {}", token))
        .json(&json!({
            "domain": "example.com",
...
async fn update_site_rule() {
    ...
    let res = app
        .client
        .patch(app.url(&format!("/api/v1/site-rules/{}", rule_id)))
        .header("Authorization", format!("Bearer {}", token))
...
async fn delete_site_rule() {
    ...
    let res = app
        .client
        .delete(app.url(&format!("/api/v1/site-rules/{}", rule_id)))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
