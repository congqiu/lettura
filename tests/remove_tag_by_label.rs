...
async fn delete_tag_by_label_removes_from_entry() {
    ...
    // attach tag
    app.client
        .post(app.url(&format!("/api/v1/entries/{id}/tags")))
        .header("Authorization", format!("Bearer {jwt}"))
    ...
    let r = app
        .client
        .delete(app.url(&format!("/api/v1/entries/{id}/tags/by-label/rust")))
        .header("Authorization", format!("Bearer {jwt}"))
        .send()
        .await
        .unwrap();
    ...
    // list tags -> empty
    let tags: Vec<serde_json::Value> = app
        .client
...
async fn delete_unknown_tag_label_returns_404() {
    ...
    let r = app
        .client
        .delete(app.url(&format!("/api/v1/entries/{id}/tags/by-label/nonexistent")))
        .header("Authorization", format!("Bearer {jwt}"))
        .send()
        .await
        .unwrap();
