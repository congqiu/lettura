...
async fn audit_log_insert_works_directly() {
    ...
    let (user_id,): (uuid::Uuid,) = sqlx::query_as("SELECT id FROM users WHERE email=$1")
        .bind("direct@e.com").fetch_one(&app.pool).await.unwrap();
    ...
    assert_eq!(log.action, lettura::models::audit_log::AuditAction::CreateEntry);
...
async fn saving_entry_creates_audit_log() {
    ...
    // Create an entry
    let resp = app.client.post(app.url("/api/v1/entries"))
    ...
    // Query audit logs
    let resp = app.client.get(app.url("/api/v1/audit-logs"))
        .header("Authorization", format!("Bearer {jwt}"))
        .send().await.unwrap();
    assert_eq!(resp.status(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
    let logs = body["data"].as_array().unwrap();
    assert!(!logs.is_empty(), "audit logs should not be empty after creating entry");
...
async fn audit_logs_support_filtering_by_action() {
    ...
    // Filter by action
    let resp = app.client.get(app.url("/api/v1/audit-logs?action=create_entry"))
        .header("Authorization", format!("Bearer {jwt}"))
        .send().await.unwrap();
    assert_eq!(resp.status(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
...
async fn audit_logs_pagination_works() {
    ...
    let resp = app.client.get(app.url("/api/v1/audit-logs?limit=2&offset=0&action=create_entry"))
        .header("Authorization", format!("Bearer {jwt}"))
        .send().await.unwrap();
    assert_eq!(resp.status(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
...
async fn audit_logs_are_isolated_per_user() {
    ...
    app.client.post(app.url("/api/v1/auth/register"))
        ...
        .send().await.unwrap();
    let login_a: serde_json::Value = app.client.post(app.url("/api/v1/auth/login"))
    ...
    app.client.post(app.url("/api/v1/auth/register"))
        ...
        .send().await.unwrap();
    let login_b: serde_json::Value = app.client.post(app.url("/api/v1/auth/login"))
    ...
    assert_eq!(resp.status(), 200);
    let body: serde_json::Value = resp.json().await.unwrap();
    assert!(body["data"].as_array().unwrap().is_empty(), "user B should have no create_entry logs");
