...
fn locate_cli_binary() -> std::path::PathBuf {
    ...
    let p = std::path::PathBuf::from(&target_dir).join(profile).join("lettura-cli");
    if !p.exists() {
        let output = Command::new(env!("CARGO"))
            .args(["build", "-p", "lettura-cli"])
            .output()
            .expect("failed to invoke cargo build");
        assert!(output.status.success(), "cargo build -p lettura-cli failed: {}",
...
async fn make_pat(app: &TestApp) -> String {
    ...
    let (user_id,): (Uuid,) = sqlx::query_as("SELECT id FROM users WHERE email=$1")
        .bind("u@e.com").fetch_one(&app.pool).await.unwrap();
    ...
    lettura::models::pat::insert(
        &app.pool, user_id, "contract-test", &hash, &prefix,
...
async fn run_cli(bin: &std::path::Path, server: &str, token: &str, args: &[&str]) -> (i32, String, String) {
...
async fn cli_whoami_hits_server() {
    ...
    let parsed: serde_json::Value = serde_json::from_str(stdout.trim())
        .expect("whoami did not emit JSON");
...
async fn cli_save_then_list_finds_entry() {
    ...
    // save
    let (code, stdout, stderr) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/a", "--tag", "backlog"]).await;
    ...
    let (code, stdout, stderr) = run_cli(&bin, &app.addr, &token,
        &["list", "--filter", "tag:backlog", "--output", "ids"]).await;
    assert_eq!(code, 0, "list failed: stderr={stderr} stdout={stdout}");
    let ids: Vec<&str> = stdout.lines().collect();
    assert!(ids.iter().any(|id| *id == entry_id), "list --output ids missing {entry_id}: {ids:?}");
    app.cleanup().await;
}
...
async fn cli_save_same_url_twice_reports_already_existed() {
    ...
    let (c1, s1, _) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/b"]).await;
    assert_eq!(c1, 0);
    let first: serde_json::Value = serde_json::from_str(s1.trim()).unwrap();
    assert_eq!(first["already_existed"], false);

    let (c2, s2, _) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/b"]).await;
    assert_eq!(c2, 0);
...
async fn cli_tag_then_get_shows_tag() {
    ...
    let (_, save_stdout, _) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/c"]).await;
    ...
    let (code, get_stdout, _) = run_cli(&bin, &app.addr, &token, &["get", id, "--format", "json"]).await;
    assert_eq!(code, 0);
    ...
    let tag_rows: Vec<serde_json::Value> = app.client
        .get(app.url(&format!("/api/v1/entries/{id}/tags")))
        .header("Authorization", format!("Bearer {token}"))
        .send().await.unwrap().json().await.unwrap();
    let labels: Vec<String> = tag_rows.iter()
        .map(|r| r["label"].as_str().unwrap().to_string()).collect();
...
async fn cli_bulk_dry_run_does_not_modify() {
    ...
    let (_, _, _) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/d"]).await;
    let (_, _, _) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/e"]).await;

    // dry-run should report matched without persisting
    let (code, stdout, stderr) = run_cli(&bin, &app.addr, &token,
        &["tag", "--add", "autotag", "--filter", "untagged", "--dry-run"]).await;
    assert_eq!(code, 0, "dry-run failed: {stderr}");
    let resp: serde_json::Value = serde_json::from_str(stdout.trim()).unwrap();
    assert!(resp["matched"].as_i64().unwrap() >= 2, "expected >=2 matched: {resp}");
    ...
    let tag_rows: Vec<serde_json::Value> = app.client
        ...
        .header("Authorization", format!("Bearer {token}"))
        .send().await.unwrap().json().await.unwrap();
    assert!(tag_rows.is_empty(), "dry-run should not have applied the tag");

    // Now --yes should apply
    let (code, stdout, _) = run_cli(&bin, &app.addr, &token,
        &["tag", "--add", "autotag", "--filter", "untagged", "--yes"]).await;
    assert_eq!(code, 0);
    let resp: serde_json::Value = serde_json::from_str(stdout.trim()).unwrap();
...
async fn cli_bulk_without_safety_flag_errors() {
    ...
    let (code, _stdout, stderr) = run_cli(&bin, &app.addr, &token,
        &["tag", "--add", "x", "--filter", "untagged"]).await;
    assert_ne!(code, 0, "expected non-zero exit without --dry-run/--yes");
    assert!(stderr.contains("bad_args") || stderr.contains("--dry-run"),
...
async fn cli_get_markdown_has_frontmatter() {
    ...
    let (_, save_stdout, _) = run_cli(&bin, &app.addr, &token,
        &["save", "https://contract-test.example/f", "--title", "Hello"]).await;
    let save_json: serde_json::Value = serde_json::from_str(save_stdout.trim()).unwrap();
    let id = save_json["id"].as_str().unwrap();

    let (code, stdout, stderr) = run_cli(&bin, &app.addr, &token,
        &["get", id, "--format", "markdown"]).await;
    assert_eq!(code, 0, "get markdown failed: {stderr}");
    assert!(stdout.starts_with("---"), "expected front-matter: first 80 chars = {}", &stdout.chars().take(80).collect::<String>());
