...
async fn login_and_create_entries(app: &TestApp, urls: &[&str]) -> (String, Vec<String>) {
    ...
    let mut ids = vec![];
    for url in urls {
        let r: serde_json::Value = app.client.post(app.url("/api/v1/entries"))
            .header("Authorization", format!("Bearer {jwt}"))
            .json(&json!({"url": url})).send().await.unwrap().json().await.unwrap();
...
async fn bulk_tag_applied() {
    ...
    let _: serde_json::Value = app.client.post(app.url("/api/v1/entries/bulk/tag"))
        .header("Authorization", format!("Bearer {jwt}"))
        .json(&json!({"filter": {"untagged": true}, "add": ["auto"]}))
        .send().await.unwrap().json().await.unwrap();

    let entries: Vec<serde_json::Value> = app.client.get(app.url("/api/v1/entries?tag=auto"))
        .header("Authorization", format!("Bearer {jwt}"))
        .send().await.unwrap().json().await.unwrap();
...
async fn bulk_archive_sets_is_archived() {
    ...
    let r: serde_json::Value = app.client.post(app.url("/api/v1/entries/bulk/archive"))
        .header("Authorization", format!("Bearer {jwt}"))
...
async fn bulk_add_empty_returns_400() {
    ...
    let r = app.client.post(app.url("/api/v1/entries/bulk/tag"))
        .header("Authorization", format!("Bearer {jwt}"))
