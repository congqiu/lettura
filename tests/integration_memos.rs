...
async fn create_and_list_memos() {
    ...
    let res = app.client.post(app.url("/api/v1/memos"))
        .header("Authorization", format!("Bearer {}", token))
        .json(&json!({"content": "remember this"})).send().await.unwrap();
    assert_eq!(res.status(), 200);
    let memo: serde_json::Value = res.json().await.unwrap();
    assert_eq!(memo["content"], "remember this");
    let res = app.client.get(app.url("/api/v1/memos"))
...
async fn delete_memo() {
    ...
    let res = app.client.post(app.url("/api/v1/memos"))
        .header("Authorization", format!("Bearer {}", token))
        .json(&json!({"content": "to delete"})).send().await.unwrap();
    let memo: serde_json::Value = res.json().await.unwrap();
    let memo_id = memo["id"].as_str().unwrap();
    let res = app.client.delete(app.url(&format!("/api/v1/memos/{}", memo_id)))
        .header("Authorization", format!("Bearer {}", token)).send().await.unwrap();
    assert_eq!(res.status(), 200);
    app.cleanup().await;
}
...
async fn promote_memo_with_url() {
    ...
    let res = app.client.post(app.url("/api/v1/memos"))
        .header("Authorization", format!("Bearer {}", token))
        .json(&json!({"content": "check out https://example.com/promoted"})).send().await.unwrap();
    let memo: serde_json::Value = res.json().await.unwrap();
    let memo_id = memo["id"].as_str().unwrap();
    let res = app.client.post(app.url(&format!("/api/v1/memos/{}/promote", memo_id)))
        .header("Authorization", format!("Bearer {}", token)).send().await.unwrap();
    assert_eq!(res.status(), 200);
    let body: serde_json::Value = res.json().await.unwrap();
...
async fn empty_memo_rejected() {
    ...
    let res = app.client.post(app.url("/api/v1/memos"))
        .header("Authorization", format!("Bearer {}", token))
        .json(&json!({"content": ""})).send().await.unwrap();
    assert_eq!(res.status(), 400);
    app.cleanup().await;
}
