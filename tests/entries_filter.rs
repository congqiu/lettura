...
async fn add_tag(pool: &sqlx::PgPool, user_id: uuid::Uuid, entry_id: uuid::Uuid, label: &str) {
...
async fn filter_since_and_before() {
    ...
    sqlx::query("UPDATE entries SET created_at = now() - INTERVAL '100 days' WHERE id = $1")
        .bind(e)
        .execute(&app.pool)
        .await
        .unwrap();
...
async fn list_rejects_excessive_page() {
    ...
    let res = app
        .client
        .get(app.url("/api/v1/entries?page=51&per_page=100"))
    ...
    let res = app
        .client
        .get(app.url("/api/v1/entries?page=50&per_page=100"))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
...
async fn list_with_cursor_returns_next_page_via_header() {
    ...
    let res = app
        .client
        .get(app.url("/api/v1/entries?per_page=2"))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
    assert_eq!(res.status(), 200);
    let next_header_first = res
        .headers()
        .get("x-next-cursor")
    ...
    assert!(
        next_header_first.is_some(),
        "X-Next-Cursor must be present on first page when there are more entries"
    );
    ...
    let res = app
        .client
        .get(app.url(&format!("/api/v1/entries?per_page=2&cursor={}", url_cursor)))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
    ...
    for item in &second {
        assert!(
            !ids_first.contains(&item["id"].as_str().unwrap()),
    ...
    assert!(
        next_header.is_some(),
        "X-Next-Cursor expected on full page in cursor mode"
    );
    ...
    let res = app
        .client
        .get(app.url(&format!(
            "/api/v1/entries?per_page=2&cursor={}",
            url_cursor2
        )))
        .header("Authorization", format!("Bearer {}", token))
        .send()
    ...
    assert_eq!(
        third.len(),
        1,
        "5 items / per_page=2: third page has 1 item"
    );
    assert!(
        final_header.is_none(),
        "X-Next-Cursor must NOT be set when page is short"
    );
...
async fn cursor_bypasses_page_50_guard() {
    ...
    let res = app
        .client
        .get(app.url(&format!("/api/v1/entries?page=999&cursor={}", url_cursor)))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
...
async fn invalid_cursor_returns_400() {
    ...
    let res = app
        .client
        .get(app.url("/api/v1/entries?cursor=garbage"))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .unwrap();
