...
- 后端编译/测试**优先使用本机 cargo**（速度快、IDE 集成好）；本机未装 Rust 时再 fallback 到 Docker。详见下方"单元测试 / 集成测试"小节
...
**优先用本机 cargo**（如果装了 Rust toolchain）：
...
# 单个模块
cargo test --lib search --no-default-features

# 集成测试（需要 PG 起着：./dev.sh up postgres）
DATABASE_URL=postgres://lettura:lettura@127.0.0.1:5436/lettura \
  cargo test --no-default-features --features test-utils --tests
```

本机未装 Rust 时用 Docker fallback：

```bash
# 单元测试（BuildKit 缓存增量编译很快）
...
# 或 ad-hoc 容器（lettura 服务未起时）:
docker run --rm --network lettura_default \
  -e DATABASE_URL=postgres://lettura:lettura@postgres:5432/lettura \
  -v "$(pwd)":/app -v lettura-target:/app/target -v lettura-cargo-registry:/usr/local/cargo/registry \
  -w /app lettura-bb-sccache cargo test --no-default-features --features test-utils
```

集成测试需要 `--features test-utils`（启用 SSRF skip 等 test-only hook，详见 fetch_worker.rs::WorkerConfig.skip_ssrf 注释）。
